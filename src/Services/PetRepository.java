...
public class PetRepository implements IRepository<Pet> {
    ...
    public Pet getById(int petId) {
        ...
        try {
            ...
            try (Connection dbConnection = getConnection()) {
                ...
                SQLst = "SELECT GenusId, Id, PetName, Birthday FROM pet_list WHERE Id = ?";
    ...
    public int create(Pet pet) {
        ...
        try {
            ...
            try (Connection dbConnection = getConnection()) {
                ...
                SQLst = "INSERT INTO pet_list (PetName, Birthday, GenusId) SELECT ?, ?, (SELECT Id FROM pet_types WHERE Genus_name = ?)";
                PreparedStatement prepSt = dbConnection.prepareStatement(SQLst);
    ...
    public int update(Pet pet) {
        ...
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            try (Connection dbConnection = getConnection()) {
                SQLst = "UPDATE pet_list SET PetName = ?, Birthday = ? WHERE Id = ?";
                PreparedStatement prepSt = dbConnection.prepareStatement(SQLst);
    ...
    public void delete (int id) {
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            try (Connection dbConnection = getConnection()) {
                SQLst = "DELETE FROM pet_list WHERE Id = ?";
                PreparedStatement prepSt = dbConnection.prepareStatement(SQLst);
                prepSt.setInt(1,id);
                prepSt.executeUpdate();
            }
